...
class ParserIlBoard(Parser):
    index = None

    def __init__(self):
        super().__init__('http://il.lboard.net/')
        self.num_threads = 0
    ...
    def parse_details(self, url):
        ...
        while not detail_url_open:
            ...
            try:
                ...
                for re_item in re.findall('Имя: (.+?)<', str(html_src), re.DOTALL):
                    curr_name = re_item
    ...
    def parse_category(self, url):
        ...
        while not (url is None):
            ...
            for item in soup.find_all('div', {"class": 'group3'}):
                for date_item in re.findall('<div class="group3">([0-9][0-9]\.[0-9][0-9]\.[0-9][0-9][0-9][0-9]).+?<div class="category">', str(item), re.DOTALL):
                    if date_item.strip() == '':
                        continue
            ...
            if old_date_reached:
                ...
                url = None
                for item in re.findall('<span class=sel>.+?<a href="(.+?)">', str(html_src), re.DOTALL):
                    url = self.base_url[:-1] + html.unescape(str(item))
                    break
    ...
    def load_phones(self):
        ...
        if main_page_open:
            ...
            cat_list = list()
            for group in cat_soup.find_all("div", class_="category"):
                for a in group.find_all('a'):
                    if a['href'] in cat_list:
                        continue
                    cat_list.append(a['href'])
                    cat_url = self.base_url[:len(self.base_url) - 1] + a['href']
